...
const getAllBookshandler = (request, h) => {
    ...
    let booksreq = books;
        if (name !== undefined) {
            booksreq = books.filter((book) => book.name.toLowerCase().includes(name.toLowerCase()));
        } 
        if (reading !== undefined) {
            booksreq = books.filter((book) => book.reading === (reading === '1'));
        } 
        if (finished !== undefined) {
            booksreq = books.filter((book) => book.finished === (finished === '1'));
        }
        const response = h.response({
            status: 'success',
            data: {
                books: booksreq.map((book) => ({
                    id: book.id,
                    name: book.name,
...
const editBookshandlerbyId = (request, h) =>{
    const { id } = request.params;
...
const deleteBookshandlerbyId  = (request, h) =>{
    const {id} = request.params;
